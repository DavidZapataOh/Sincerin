...
//
// Source: https://github.com/miquelcabot/ultrahonk_verifier/blob/main/src/lib.rs
...
fn verify_shplemini<H: CurveHooks>(
    ...
    // - Compute vector (r, r^2, ..., r^{2^(n-1)}), where n := log_circuit_size
    ...
    // VK commitments assigned to commitments array (indices offset by 1 for ZK, 0 for Plain)
    ...
    // Proof point commitments
    commitments[28 + offset] = convert_proof_point(*parsed_proof.w1())?;
    commitments[29 + offset] = convert_proof_point(*parsed_proof.w2())?;
    commitments[30 + offset] = convert_proof_point(*parsed_proof.w3())?;
    commitments[31 + offset] = convert_proof_point(*parsed_proof.w4())?;
    commitments[32 + offset] = convert_proof_point(*parsed_proof.z_perm())?;
    commitments[33 + offset] = convert_proof_point(*parsed_proof.lookup_inverses())?;
    commitments[34 + offset] = convert_proof_point(*parsed_proof.lookup_read_counts())?;
    commitments[35 + offset] = convert_proof_point(*parsed_proof.lookup_read_tags())?;

    // to be Shifted - copied to avoid re-validation
    commitments[36 + offset] = commitments[28 + offset]; // w1
    commitments[37 + offset] = commitments[29 + offset]; // w2
    commitments[38 + offset] = commitments[30 + offset]; // w3
    commitments[39 + offset] = commitments[31 + offset]; // w4
    commitments[40 + offset] = commitments[32 + offset]; // z_perm
    ...
    // Compute Shplonk constant term contributions from gemini fold commitments
    ...
    // ZK-specific: Libra polynomial evaluation batch opening
    if let Transcript::ZK(zktp) = tp {
        // ... (libra commitments and evaluations processing)
        // See full source for details
    }

    // Generator point for constant term
    commitments[boundary] = G1::<H>::generator(); // (1, 2)
    scalars[boundary] = constant_term_accumulator;
    boundary += 1;

    // KZG quotient commitment
    let quotient_commitment = convert_proof_point(*parsed_proof.kzg_quotient())?;
    ...
    // PAIRING CHECK
    // ==========================================================================
    // P_0 = MSM(commitments, scalars)
    // P_1 = -kzg_quotient
    // Check: e(P_0, SRS_G2) * e(P_1, SRS_G2_VK) == 1
    ...
    let g2_points = [
        G2Prepared::from(read_g2::<H>(&SRS_G2)?),
        G2Prepared::from(read_g2::<H>(&SRS_G2_VK)?),
    ];
